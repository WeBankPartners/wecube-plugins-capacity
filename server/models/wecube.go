...
func refreshToken() error {
	req,err := http.NewRequest(http.MethodGet, Config().DataSource.Monitor.BaseUrl + "/auth/v1/api/token", strings.NewReader(""))
	if err != nil {
...
func requestCoreToken(rsaKey string) error {
	...
	fmt.Printf("param: %s \n", string(postBytes))
	req,err := http.NewRequest(http.MethodPost, Config().DataSource.Monitor.BaseUrl + "/auth/v1/api/login", bytes.NewReader(postBytes))
	if err != nil {
		return fmt.Errorf("http new request fail,%s ", err.Error())
	}
	resp,err := http.DefaultClient.Do(req)
...
func GetCoreToken() string {
	if Config().DataSource.Monitor.BaseUrl == "" {
		return ""
	}
